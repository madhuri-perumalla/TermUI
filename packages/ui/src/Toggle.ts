...
import { type Screen, type KeyEvent, mergeStyles, defaultStyle, styleToCellAttrs } from '@termuijs/core';
...
export class Toggle extends Widget {
    ...
    protected _renderSelf(screen: Screen): void {
        ...
        const toggleText = this._value ? '[ ON ]' : '[OFF]';

        const text = this._label
            ? `${this._label} ${toggleText}`
            : toggleText;
