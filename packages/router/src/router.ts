...
export interface RouterOptions {
    ...
    maxHistory?: number;
    /** Maximum redirect depth before throwing error (default: 10) */
    maxRedirects?: number;
}
...
export class Router {
    ...
    constructor(options: RouterOptions = {}) {
        ...
        this._maxRedirects = options.maxRedirects ?? 10;
    ...
    public wrapScreen(match: RouteMatch): VNode {
    ...
    private _navigateTo(path: string): void {
        ...
        if (typeof guardResult === 'string') {
            this._redirectDepth++;
            if (this._redirectDepth > this._maxRedirects) {
                this.events.emit('error', new Error(`Maximum redirect depth (${this._maxRedirects}) exceeded. Possible infinite redirect loop.`));
